  ...
  return (
    <>
      <nav className="fixed top-4 md:top-6 left-1/2 -translate-x-1/2 z-[60] w-[92%] max-w-7xl glass-vibrant py-3 md:py-4 px-6 md:px-10 flex justify-between items-center rounded-full border border-accent-primary/20 premium-card-shadow">
        <Link href="/" className="flex items-center gap-2 md:gap-4 group cursor-pointer">
          ...
          <div className="flex flex-col -space-y-1">
            <span className="text-sm md:text-xl font-display tracking-[0.1em] text-studio-text">NEWLAND</span>
        ...
        {/* Desktop Navigation */}
        <div className="hidden lg:flex gap-12 text-[10px] font-bold tracking-[0.2em] uppercase text-zinc-muted">
          <Link href="/#services" className="hover:text-accent-primary transition-all duration-300 relative group">
            SERVICES
          </Link>
          <Link href="/products" className="hover:text-accent-primary transition-all duration-300 relative group">
            PRODUCTS
          </Link>
        ...
        <div className="flex items-center gap-3 md:gap-6">
          ...
          <button 
            ...
            className="lg:hidden w-10 h-10 flex flex-col items-center justify-center gap-1.5 glass-vibrant rounded-full border border-accent-primary/20"
          >
            <span className={`w-5 h-[2px] bg-accent-primary transition-all ${isMenuOpen ? 'rotate-45 translate-y-2' : ''}`} />
            <span className={`w-5 h-[2px] bg-accent-primary transition-all ${isMenuOpen ? 'opacity-0' : ''}`} />
            <span className={`w-5 h-[2px] bg-accent-primary transition-all ${isMenuOpen ? '-rotate-45 -translate-y-2' : ''}`} />
          </button>
        </div>
      </nav>
      ...
      <div className={`fixed inset-0 z-[55] lg:hidden transition-all duration-500 ${isMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        <div className="absolute inset-0 bg-studio-bg/80 backdrop-blur-2xl" onClick={() => setIsMenuOpen(false)} />
        <div className={`absolute top-24 left-1/2 -translate-x-1/2 w-[92%] glass-vibrant rounded-[2.5rem] border border-accent-primary/20 p-12 transition-all duration-500 premium-card-shadow ${isMenuOpen ? 'translate-y-0 opacity-100' : '-translate-y-10 opacity-0'}`}>
          <div className="flex flex-col items-center gap-10 text-center">
            <div className="space-y-8 text-sm font-bold tracking-[0.4em] uppercase text-zinc-muted">
              <Link href="/#services" onClick={() => setIsMenuOpen(false)} className="block hover:text-accent-primary transition-colors">Services</Link>
              <Link href="/products" onClick={() => setIsMenuOpen(false)} className="block hover:text-accent-primary transition-colors">Products</Link>
            ...
            <div className="w-full pt-8 border-t border-accent-primary/10">
              <Link
                href="/#contact"
                onClick={() => setIsMenuOpen(false)}
                className="block w-full py-6 modern-gradient text-white text-[10px] font-bold uppercase tracking-[0.4em] rounded-2xl shadow-xl shadow-accent-primary/20"
              >
                START PROJECT
              </Link>
            </div>
